...
from Local.AudioDeviceList import checkSamplingRate, list_audio_device
import time
...
class ServerDeviceSettings:
    ...
    serverMonitorDeviceId: int = -1  # -1 disables monitor
...
class ServerDevice:
    def __init__(self, serverDeviceCallbacks: ServerDeviceCallbacks):
        ...
        # For setting change confirmation
    ...
    def start(self):
        ...
        while True:
            if self.settings.serverAudioStated == 0 or self.settings.serverInputDeviceId == -1:
                ...
                # sampling rate
                # Unify to the same sampling rate (because there may be a shortage of samples during conversion.If the padding method is clarified, it may be possible to set each)
