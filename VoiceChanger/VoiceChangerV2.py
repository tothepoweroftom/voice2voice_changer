"""
■ VoiceChangerV2
- Difference with VoiceChanger
・Transfer resample processing to VoiceChangerModel to avoid wasteful resample processing
・Eliminated splitting of preprocessing and main processing (to avoid unnecessary type conversion in VoiceChangeModel)

- Applicable VoiceChangerModel
・DiffusionSVC
・RVC
"""
...
class VoiceChangerV2(VoiceChangerIF):
    ...
    def _generate_strength(self, crossfadeSize: int):
        if (
            ...
            or self.currentCrossFadeOffsetRate
            != self.settings.crossFadeOffsetRate
            or self.currentCrossFadeEndRate != self.settings.crossFadeEndRate
            or self.currentCrossFadeOverlapSize
            != self.settings.crossFadeOverlapSize
        ):
            self.crossfadeSize = crossfadeSize
            self.currentCrossFadeOffsetRate = self.settings.crossFadeOffsetRate
            self.currentCrossFadeEndRate = self.settings.crossFadeEndRate
            self.currentCrossFadeOverlapSize = (
                self.settings.crossFadeOverlapSize
            )
            ...
            # Since the size differs from the previous result, the record is deleted.
    ...
    def on_request(
        ...
        try:
            ...
            with Timer("main-process") as t:
                ...
                if self.noCrossFade:  # Beatrice
                    ...
                    if hasattr(self, "sola_buffer") is True:
                        ...
                        output_wav = audio[sola_offset:sola_end].astype(
                            np.float64
                        )
            ...
            # Since fallback processing is done in RVC.py, we just return dummy data here.
            return np.zeros(1).astype(np.int16), [0, 0, 0]
