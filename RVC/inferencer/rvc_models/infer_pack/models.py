...
class Generator(torch.nn.Module):
    def __init__(
        ...
        if resblock != "1":
            ...
            for i in range(len(self.ups) - 1, -1, -1):
                for k, d in zip(
                    resblock_kernel_sizes[::-1], resblock_dilation_sizes[::-1]
                ):
                    # conv2
                ...
                # upsampling
                self.ups_size[i] = (
                    -(-self.ups_size[i] // upsample_rates[i])
    ...
    def infer_realtime(self, x, g=None, convert_length=None):
        ...
        for i in range(self.num_upsamples):
            if self.realtime:
                x = x[
                    ...
                    :,
                    -self.ups_size[i]
                    + (-convert_length // np.prod(self.upsample_rates[i:])) :,
                ]
            x = F.leaky_relu(x, LRELU_SLOPE)
            x = self.ups[i](x)
        ...
        x = torch.tanh(x)
        out = torch.zeros(
            [x.shape[0], 1, out_length], device=x.device, dtype=x.dtype
        )
...
class GeneratorNSF(torch.nn.Module):
        ...
        if resblock != "1":
            ...
            for i in range(len(self.ups) - 1, -1, -1):
                for k, d in zip(
                    ...
                    self.ups_size[i] += d[-1] * (k - 1) // 2
                # noise_conv
                self.noise_conv_size[i] = self.ups_size[i] * np.prod(
                    upsample_rates[i:]
                )
                # upsampling
    ...
    def infer_realtime(self, x, f0, g=None, convert_length=None):
        ...
        for i in range(self.num_upsamples):
            ...
            x = F.leaky_relu(x, LRELU_SLOPE)
            x_ = self.ups[i](x)
            x_source = self.noise_convs[i](
                har_source[:, :, -convert_length - self.noise_conv_size[i] :]
            )
            x = torch.zeros(
                [x_.shape[0], x_.shape[1], max(x_.shape[2], x_source.shape[2])],
                device=x.device,
                dtype=x.dtype,
            )
            x[:, :, -x_.shape[2] :] += x_
