...
class Pipeline(object):
    ...
    def exec(
        ...
        with Timer2("Pipeline-Exec", False) as t:  # NOQA
            # It comes in at a sampling rate of 16,000. After that, this world will be processed at 16000.
            search_index = self.index is not None and self.big_npy is not None and index_rate != 0
            # self.t_pad = self.sr * repeat   # translate to 1 second
            # self.t_pad_tgt = self.targetSR * repeat  # 1 second Trimming at output (output by model sampling)
            audio = audio.unsqueeze(0)

            quality_padding_sec = (repeat * (audio.shape[1] - 1)) / self.sr  # padding(reflect) The size of the file must be smaller than the original size.

            self.t_pad = round(self.sr * quality_padding_sec)  # Add audio before and after
            self.t_pad_tgt = round(self.targetSR * quality_padding_sec)  # Add audio before and after; trimming when outputting (outputted by sampling the model)
            audio_pad = F.pad(audio, (self.t_pad, self.t_pad), mode="reflect").squeeze(0)
            p_len = audio_pad.shape[0] // self.window
            sid = torch.tensor(sid, device=self.device).unsqueeze(0).long()

            # Turn off silence_front when RVC Quality is On.
            ...
            # If the pitch estimation does not work well (pitchf = 0), mix the features before searching
            # I have some questions about how to create pitchff, but since it is the same as the original, I will use it as it is.
            ...
            # The sampling rate output by infer is the sampling rate of the model.
            # When input to pipeline, it is 16k like hubert 
            if self.t_pad_tgt != 0:
