...
class DeviceManager(object):
    ...
    def getOnnxExecutionProvider(self, gpu: int):
        ...
        if gpu >= 0 and "CUDAExecutionProvider" in availableProviders and devNum > 0:
            if gpu < devNum:  # You can play it with the previous if statement, but to increase the resolution of the error, move it down one step.
    ...
    def halfPrecisionAvailable(self, id: int):
        ...
        if cap[0] < 7:  # It is said that half precision can be used if the computing function is 7 or higher (but there are exceptions? T500 etc.)
            return False
